...
bool cmpf(float &p1, float &p2)
{
    return p1< p2;
}

float median(vector<float> c)
{
    std::sort(c.begin(),c.end());
    if (c.size() % 2 ==0) return (c[c.size()/2 - 1] + c[c.size()/2]) / 2;
...
Chordino::getRemainingFeatures()
{
    ...
    **/
    ...
    vector<vector<double> > chordogram1;
    ...
    for (FeatureList::iterator it = tunedSpec.begin(); it != tunedSpec.end(); ++it) {
        ...
        // vector<float>::const_iterator first = chroma.begin() + 12;
        //     vector<float>::const_iterator last = chroma.end() ;
        //     vector<float> temp1(first, last);
        //     chroma_features.push_back(temp1);
// chroma_features.push_back(chroma);
    ...
    cerr << "done." << endl;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    // for (int iframe = 0 ; iframe < chroma_features.size() ; iframe++)
    // {
    //     vector<float> new_chroma(12,0.0);
        
    //     for(int ichroma = 0 ; ichroma<new_chroma.size() ; ichroma++)
    //     {
    //         int index = iframe + ichroma -5;
    //         if(index <0) index += chroma_features.size();
    //         if (index >= chroma_features.size()) index %= chroma_features.size();
    //         new_chroma[ichroma] = (median(chroma_features[index]));
    //     }
    //     // for (int j = 0 ; j<new_chroma.size();j++)
    //     // {
    //     //     cerr << new_chroma[j] << " ";
    //     // }
    //     // cerr << endl;
    ...
    //         for (int iBin = 12; iBin < 24; iBin++) {
    //             tempchordvalue += m_chorddict[24 * iChord + iBin] * new_chroma[iBin-12];
    //         }
    ...
    //         currentChordSalience[nChord-1] = 1.0;
    //     }
    //     chordogram1.push_back(currentChordSalience);
...
//cerr << endl;
	...
	if (m_useHMM == 1.0) {
        ...
        vector<int> chordpath = ViterbiPath(init, trans, chordogram1, delta,m_chordnames,chords);
